...
<Response>
  ...
  async inboundVoiceStreamStatus(req, res) {
    ...
    if (/stop/i.test(streamEvent) && callSid && callSid !== "-") {
      finalizeInboundCallRecord(callSid).catch((err) => {
        // eslint-disable-next-line no-console
        console.error(
          `[Twilio][stream-status] finalize call failed callSid=${callSid}: ${err.message}`
        );
      });
    }
