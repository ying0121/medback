...
/**
 * Mark an inbound call complete and set duration from elapsed time when Twilio
...
async function finalizeInboundCallRecord(callSid, { status = "completed" } = {}) {
  ...
  let call = await Call.findOne({ where: { callSid: sid } });
  if (!call) {
    call = await Call.create({
      callSid: sid,
      phone: "unknown",
  ...
  if (finalSeconds !== null && finalSeconds > Number(call.seconds || 0)) {
